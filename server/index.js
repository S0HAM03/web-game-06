...
const { initWords, getRandomWord, isValidWord } = require('./words');
...
io.on('connection', (socket) => {
  ...
  socket.on('create_room', ({ playerName }) => {
    if (!playerName?.trim()) { socket.emit('room_error', { message: 'Please enter your name.' }); return; }
  ...
  socket.on('join_room', ({ roomCode, playerName }) => {
    if (!playerName?.trim()) { socket.emit('room_error', { message: 'Please enter your name.' }); return; }
    if (!roomCode?.trim()) { socket.emit('room_error', { message: 'Please enter a room code.' }); return; }
  ...
  socket.on('start_game', () => {
    ...
    io.to(result.roomCode).emit('game_start', {
      ...
      isSinglePlayer: result.room.players.length === 1,
    });
  ...
  socket.on('submit_guess', ({ guess }) => {
    ...
    if (!result.success) { socket.emit('guess_error', { message: result.message }); return; }
    ...
    if (result.gameOver) {
      const playerBoards = result.room.players.map(p => ({
        id: p.id, name: p.name, solved: p.solved,
        attempts: p.attempts, guesses: p.guesses, solvedAt: p.solvedAt,
      }));
  ...
  socket.on('play_again', () => {
    ...
    if (!result.success) { socket.emit('room_error', { message: result.message || 'Could not reset.' }); return; }
    io.to(result.room.code).emit('rematch_ready', { playerList: getPlayerList(result.room) });
    console.log(`[REMATCH] ${result.room.code}`);
  });

  socket.on('disconnect', () => {
    const result = removePlayer(socket.id);
    if (!result || result.destroyed) { console.log(`[-] Disconnected: ${socket.id} (room destroyed)`); return; }
    io.to(result.roomCode).emit('player_left', {
      playerList: getPlayerList(result.room),
      newHostId: result.newHostId,
    });
    if (result.room.status === 'playing' && result.room.players.length === 1) {
      const last = result.room.players[0];
      io.to(result.roomCode).emit('game_over', {
        winnerId: last.id, winnerName: last.name,
        word: result.room.word, reason: 'opponent_left',
        playerBoards: [{ id: last.id, name: last.name, solved: last.solved, attempts: last.attempts, guesses: last.guesses, solvedAt: last.solvedAt }],
      });
...
// ── Start server after initialising word lists ─────────────────
async function main() {
  await initWords();
  server.listen(PORT, () => {
